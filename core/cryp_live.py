...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            "Tracks academic assignments, projects, and deadlines. "
            "Use this when the user wants to add an assignment, mark one "
            "complete, or asks what's due, what's pending, or what's overdue. "
            "ALWAYS use this instead of save_memory for assignment/deadline data."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action":   {"type": "STRING", "description": "add | complete | list | upcoming | overdue"},
                "title":    {"type": "STRING", "description": "Assignment or project title"},
                "course":   {"type": "STRING", "description": "Course or subject name"},
                "due_date": {"type": "STRING", "description": "Due date, any format (e.g. 2026-06-20, 'next Friday')"},
                "priority": {"type": "STRING", "description": "low | normal | high (default: normal)"},
            },
            "required": ["action"]
        }
...
class CrypLive:
    ...
    def _do_shutdown(self):
        ...
        def _shutdown():
            import time, os
            from dashboard.server import cleanup_uploads
            try:
