...
def get_dashboard_summary() -> dict:
    try:
        ...
        hour_buckets = {i: [] for i in range(24)}
        for it in closed_trades:
            if not it.get("opened_at") or it["realized_r"] is None:
                continue
            try:
                dt_obj = datetime.fromisoformat(it["opened_at"])
            except ValueError:
                continue
            weekday_buckets[dt_obj.weekday()].append(it)
            hour_buckets[dt_obj.hour].append(it)
        ...
        def _bucket_stats(trades):
            ...
            return {
                "trades": len(trades),
                "win_rate": round(len(wins) / len(trades) * 100, 1),
                "avg_r": round(sum(r_vals) / len(r_vals), 2) if len(r_vals) >= 2 else None,
            }

        by_weekday = [
            {"day": i, "name": WEEKDAY_NAMES[i], **_bucket_stats(weekday_buckets[i])}
            for i in range(7)
        ]
        by_hour = [
            {"hour": i, "label": f"{i:02d}:00", **_bucket_stats(hour_buckets[i])}
            for i in range(24)
        ]
        time_analysis = {"by_weekday": by_weekday, "by_hour": by_hour}
        daily_buckets = defaultdict(lambda: {"trades": 0, "pnl": 0.0})
        for it in closed_trades:
            if not it.get("closed_at"):
                continue
            day = it["closed_at"][:10]
            daily_buckets[day]["trades"] += 1
            daily_buckets[day]["pnl"] = round(daily_buckets[day]["pnl"] + it["realized_pnl_usd"], 2)
        daily_activity = [
