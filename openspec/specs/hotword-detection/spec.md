...
The system SHALL use the openWakeWord library to continuously listen for the phrase "Hey Cryp" from the microphone and emit a wake event upon detection.

#### Scenario: Successful wake word detection
- **WHEN** the user says "Hey Cryp" within 2 meters of the microphone
- **THEN** the system SHALL detect the wake word within 1 second and emit a wake event

#### Scenario: False acceptance
- **WHEN** a phrase acoustically similar to "Hey Cryp" is spoken (e.g., "Hey Charles", "Hey Harvard")
...
- **WHEN** CrypLive is in ACTIVE state and the user says "Hey Cryp"
...
- **WHEN** CrypLive transitions from ACTIVE to SLEEPING state (session closed)
...
- **WHEN** a wake event is fired while CrypLive is in SLEEPING state
- **THEN** CrypLive SHALL exit the sleep loop and initiate a new Gemini Live session within 3 seconds
