...
- The existing **PyQt6 UI** (`CrypUI`/`MainWindow`) remains **completely unchanged**
- All existing **`CrypLive` interfaces** (`_execute_tool`, tool signatures, threading model, state machine) remain **unchanged**
...
- **Modified files**: `main.py` (wire up event bus, start dashboard thread; no interface changes to `CrypLive`), `requirements.txt` (fastapi, uvicorn, websockets already present — no new dependencies)
