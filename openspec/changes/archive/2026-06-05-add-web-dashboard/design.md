...
The `main()` function starts the PyQt6 event loop on the main thread and runs `CrypLive`'s async event loop in a daemon thread. `CrypLive` already integrates a `SystemHealthDaemon` as a background asyncio task. The codebase uses `fastapi` and `uvicorn` (already in `requirements.txt`) but has no custom WebSocket server or event bus — coordination uses `pyqtSignal`, `asyncio.Event`, and `threading.Event`.

This design adds a lightweight, read-only web dashboard that mirrors key state without modifying any existing `CrypLive` interface or the PyQt6 UI.
...
4. **Wire `CrypLive` to the event bus via `write_log` and a new `publish_state` hook** — The `DashboardEventBus` is created in `main()` and passed to `CrypLive`'s constructor. Inside `CrypLive`, `self.ui.write_log` calls are already the central logging mechanism — the event bus subscribes to the same log output. For state changes (LISTENING/THINKING/SPEAKING/SLEEPING), a thin `publish_state()` method is added to `CrypLive` that the existing state transitions already call. This approach requires zero refactoring of existing methods. Alternative considered: monkey-patching `ui.write_log` — rejected as fragile.
