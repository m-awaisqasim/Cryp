...
## 2. Wiring into CrypLive

- [x] 2.1 Import `SystemHealthDaemon` in `main.py`
- [x] 2.2 Add `self._health_daemon = SystemHealthDaemon(speak=self.speak, write_log=self.ui.write_log)` in `CrypLive.__init__`
- [x] 2.3 Add `tg.create_task(self._health_daemon.run())` alongside the other 5 tasks in `CrypLive.run()`
...
- [x] 3.3 Temporarily lower `HEALTH_CPU_THRESHOLD=5` and confirm Cryp speaks the alert
