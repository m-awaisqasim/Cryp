...
**Non-Goals:**
- No changes to `main.py`'s `CrypLive` class logic, tool dispatch, or session management
...
The replacement must expose these exact properties/methods (matching `CrypUI`):
...
| **`main.py` mainloop change** — `app.exec()` → `uvicorn.run()` is blocking but `CrypUI.__init__` no longer creates a `QApplication` | `uvicorn.run()` runs in the main thread; all existing threading in `main.py` (CrypLive runner thread, health daemon, etc.) continues unchanged |
...
| **`wait_for_api_key()` blocks** — replaced by `/setup` page | `CrypUI.__init__` no longer blocks; instead, the server returns `setup_required` WS message and `/setup` page is served for initial config |
